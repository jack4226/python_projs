...
def encode_privkey(priv, formt):
    ...
    if formt == 'decimal':
        ...
        return bin_to_b58check(encode(priv, 256, 32), 128)
    elif formt == 'wif_compressed':
        return bin_to_b58check(encode(priv, 256, 32) + '\x01', 128)
    else:
        raise Exception("Invalid format!")
...
#### - end of pybitcointools\main.py, now here's the dice stuff

MAXKEY = int('FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141', base=16)


def getrolls(fsides, frolls):
    print ("Need %s rolls of %s-sided dice. (%s bits)" % (frolls, fsides, math.log(math.pow(fsides, frolls), 2)))

    froll_list = []
    while len(froll_list) < frolls:
        print ("\nNumber of dice rolls so far: %s, need %s more." % (len(froll_list), frolls - len(froll_list)))
        if fsides < 10:
            rollinput = raw_input('Input some dice rolling results, no space between rolls:)\n>')
            rollinput = list(rollinput)
        else:
            rollinput = raw_input('Input some dice rolling results,separated by spaces:)\n>')
        ...
        for inputitem in rollinput:
            if not inputitem:
                continue
            if inputitem == 'g':
                print ("Generating random *insecure* dice rolls for remainder")
                add_rolls.extend(
                    [random.randint(1, fsides) for xx in xrange(frolls - len(froll_list) - len(add_rolls))])
                continue
            elif inputitem == 'z':
                print ("Generating minimum (zero) dice rolls for remainder")
                add_rolls.extend([1 for xx in xrange(frolls - len(froll_list) - len(add_rolls))])
                continue
            elif inputitem == 'm':
                print ("Generating maximum value dice rolls for remainder")
                add_rolls.extend([fsides for xx in xrange(frolls - len(froll_list) - len(add_rolls))])
                continue
            elif not inputitem.isdigit() or int(inputitem) < 1 or int(inputitem) > fsides:
                print ("**invalid dice roll detected: '%s', discarding all input " % inputitem)
                break
            add_rolls.append(int(inputitem))

        froll_list.extend(add_rolls)
    return froll_list


def rolls_to_hex64(rroll_list, rsides):
    if len(rroll_list) < int(math.ceil(math.log(MAXKEY, rsides))):
        print ("Dire warning: less rolls than required for secure key generation!")
        raise
    privkeyval = 0
    for roll_exp in range(0, len(rroll_list)):
        privkeyval += (rroll_list[roll_exp] - 1) * rsides ** roll_exp
        #print "Uncutkey :" + "{0:064x}".format(privkeyval)
    hexkeyout = "{0:064x}".format(privkeyval)[-64:]
    #print "hexkeyout:" + hexkeyout
    return hexkeyout


def string_scrambler(s):
    l = list(s)
    random.shuffle(l)
    scrambled = ''.join(l)
    return scrambled
...
def hexbit_scrambler(s):
    inlen = len(s)
    beano = bin(int(s, 16))[2:].zfill(len(s) * 4)
    string_scrambler(beano)
    hexkeyout = "{0:0{1}x}".format(int(beano, 2), inlen)
    return hexkeyout


def hex_randomizer(s):
    osrand = random.SystemRandom(s)
    randnum = osrand.randint(0, (2 ** (len(s) * 4)) - 1)
    sint = int(s, 16)
    xored = randnum ^ sint
    hexout = "{0:0{1}x}".format(xored, len(s))[-len(s):]
    return hexout


def hexkey_isvalid(hexkey):
    if len(hexkey) != 64 \
            or not all(c in '0123456789abcdefABCDEF' for c in hexkey) \
            or int(hexkey, base=16) > MAXKEY \
            or int(hexkey, base=16) < 1:
        return False
    return True


def print_wallet(priv):

    priv58 = encode_privkey(priv, 'wif')
    pub = privkey_to_pubkey(priv)  # uncompressed hex
    addr = privkey_to_address(priv)  # base58
    priv58c = encode_privkey(priv, 'wif_compressed')
    addrc = privkey_to_address(priv58c)  # compressed address
    pubc = encode_pubkey(pub, 'hex_compressed')

    print ("\n====== Generated Address Information ============")
    print ("Bitcoin Address:\n ", addr)
    print ("Private Key (Wallet Import Format):\n " + priv58)
    print ("Private Key (Hexadecimal 64 char) :\n " + priv)
    print ("Public Key (Full 130 character): \n " + pub)
    print ("---------\nBitcoin Address (Compressed):\n " + addrc)
    print ("Private Key (Wallet Import Format Compressed):\n " + priv58c)
    print ("Public Key (Compressed):\n " + pubc)
    print ("===================================================")


quit_input = ''
while __name__ == "__main__" and (quit_input != 'c' and quit_input != 'C'):
    ...
    roll_list = getrolls(sides, rolls)

    print ("\ndice used:")
    rolls_per_line = 16
    for i in xrange(0, rolls, rolls_per_line):
        print (roll_list[i: i + rolls_per_line - 1])

    privkey_temp = rolls_to_hex64(roll_list, sides)
    #print "\nraw dice key: \n", privkey_temp

    random.shuffle(roll_list)
    privkey_hex = rolls_to_hex64(roll_list, sides)
    privkey_hex = hexbit_scrambler(privkey_hex)
    privkey_hex = hex_randomizer(privkey_hex)
    privkey_hex = string_scrambler(privkey_hex)

    if not hexkey_isvalid(privkey_hex):
        print ('*** Wow, invalid key, try again! ***')
        continue
    else:
        print_wallet(privkey_hex)

    quit_input = raw_input('(Press "Enter" for more dice keys, "C" and "Enter" to close.)\n')
